...
const handleDeleteSector = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({
      message: "sector deleted successfully to recover go to trash page",
    });
  } catch (error) {
    console.error(error);
...
const handleRestoreSector = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({
      message: "sector recovered successfully to view go to original page",
    });
  } catch (error) {
    console.error(error);
    return res.status(500).json({ message: "Server error" });
...
const handleAllDeletedSectors = async (req, res) => {
  try {
    const sectors = await Sector.findAll({
      ...
      //   },
      // ],
      attributes: ["sector_id", "name", "deletionAt", "deletedBy"],
    });
    if (sectors.length == 0) {
