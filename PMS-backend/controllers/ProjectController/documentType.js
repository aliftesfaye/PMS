...
const handleNewDocumentType = async (req, res) => {
  ...
  if (!document_type) {
    return res
      .status(400)
      .json({ message: "Please provide required document type information" });
  }
  try {
    ...
    return res
      .status(201)
      .json({ message: "New document type created", document: documentType });
  } catch (error) {
    console.error(error);
...
const handleUpdateDocumentType = async (req, res) => {
  ...
  if (!document_type) {
    return res
      .status(400)
      .json({ message: "Please provide document type properly" });
  }
  try {
    const documentType = await DocumentType.findByPk(id);
    if (!documentType) {
...
module.exports = {
  handleNewDocumentType,
  handleGetAllDocumentTypes,
  handleGetDocumentTypeById,
